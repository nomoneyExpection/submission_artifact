...
signals, and semantic-coherence heuristics. We evaluate multiple detector
families on the same protocol --- including fine-tuned CodeBERT and
GraphCodeBERT --- and
...
An auxiliary fixed-split proxy comparison against a vulnerability detector
trained on the same features is consistent with a backdoor-specific signal (AUC
0.9997 vs.\ 0.6925 for the vulnerability detector).
...
\item \textbf{A rigorous empirical result with practical security implications.}
  ...
  A controlled fixed-split proxy comparison further suggests that the detection
  signal reflects trigger--payload coupling more than generic vulnerability
  patterns alone.
...
believing it is more secure~\cite{perry2023insecure}, and that a significant
fraction of Copilot-generated code contains
vulnerabilities~\cite{pearce2022copilot}, underscores the urgency of automated
detection.
...
Chakraborty \textit{et al.}~\cite{chakraborty2022reveal} further demonstrate
(via their ReVeal model) that combining graph neural networks with pre-trained
embeddings improves detection on real-world datasets.
...
We provide auxiliary evidence for this gap in \S\ref{sec:exp:bdvuln}
...
Derived from the Abstract Syntax Tree (AST) of the code, including tree depth,
dangerous API frequency, dynamic imports, and nested-call depth.
Backdoored code tends to exhibit deeper AST structures and more dangerous API
usage due to injected payload functions.
...
\end{equation}
The architecture supports a load-balance auxiliary loss, but all reported LOAO
runs set $\lambda{=}0$ because preliminary experiments showed no benefit from
non-zero $\lambda$ under the current routing configuration.
...
This reinforces that cross-attack evaluation should be read as a distribution of
...
\subsection{Backdoor Detection vs.\ Vulnerability Detection (Auxiliary)}
...
As an auxiliary proxy experiment on the original fixed train/val split
(not the LOAO protocol), we compare two learned detectors trained on the same
29D feature space and include Bandit as an external rule-based reference
(Table~\ref{tab:bdvuln}).

\begin{table}[t]
\centering
\caption{Auxiliary fixed-split proxy comparison: backdoor detection vs.\ vulnerability
detection. This experiment uses the original train/val split, not the LOAO
protocol. ``Vuln labels'' = trained to predict whether code contains a known
...
whereas the backdoor detector reaches 0.9997 AUC on backdoor identification.
These fixed-split proxy results are consistent with backdoor samples carrying
signals beyond the vulnerability payload itself: the co-occurrence of a
...
suggesting that rule-based scanners are not, by themselves, a substitute for
learned backdoor detectors.
...
despite being the smallest group. This suggests that token-frequency deviation
...
\midrule
\textbf{Fast screening} & Logistic Regression & 0.851 & \textbf{0.716} & \textbf{0.13$\mu$s} \\
\textbf{Interpretable MoE} & \sys{} ($N{=}1$) & 0.929 & 0.669 & 0.68$\mu$s \\
\textbf{Best AUC} & XGBoost & \textbf{0.950} & 0.707 & 10.69$\mu$s \\
\bottomrule
\end{tabular}
\end{table}
...
(MoE ablations, threshold studies, and the backdoor-vs-vulnerability proxy
comparison) still come from older fixed-split evaluations rather than the main
LOAO protocol.
...
ensemble studies, are validation-limited, and the backdoor-vs-vulnerability
comparison uses proxy vulnerability labels derived from known patterns rather
than an independent vulnerability benchmark; we therefore use these analyses
only to characterize failure modes and supporting evidence rather than as
...
\item \textbf{The detection signal is not reducible to generic vulnerability
  patterns.}
  An auxiliary fixed-split proxy experiment suggests that a vulnerability
  detector trained on the same features achieves only AUC 0.693 on backdoor
  samples, compared to 0.9997 for the backdoor detector. This 30.7-point gap is
  consistent with the learned signal being specific to the trigger--payload
  coupling.
...
\begin{center}
\url{https://anonymous.4open.science/r/submission_artifact-46CD/}
\end{center}
...
\item \textbf{Experiment code.} Claude assisted in writing the CodeBERT/GraphCodeBERT
  ...
  auxiliary backdoor-vs-vulnerability proxy comparison script. All experimental
